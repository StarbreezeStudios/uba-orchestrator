...
$ubaAgentDirectory = Join-Path $installRoot 'bin'
$ubaAgent = Join-Path $ubaAgentDirectory 'UbaAgent.exe'
$logDir = Join-Path $installRoot 'logs\helper'
$taskName = 'UbaOrchestratorHelper'
$orchestratorUrl = 'http://helsinki:8080'
$listenPort = 1346
$ubaAgentSource = '\\devopsfs.starbreeze.com\DevOps\Software-Installs\UbaAgent\UbaAgent.exe'
...
if (-not (Test-Path -LiteralPath $ubaAgentSource -PathType Leaf)) {
    throw "UbaAgent.exe was not found on the software share: $ubaAgentSource"
}
...
Write-Host "Copying UBA agent from $ubaAgentSource"
...
New-Item -ItemType Directory -Path (Split-Path $agentScript), $ubaAgentDirectory, $logDir -Force | Out-Null
Copy-Item -LiteralPath $sourceScript -Destination $agentScript -Force
Copy-Item -LiteralPath $ubaAgentSource -Destination $ubaAgent -Force
Write-Host "Installed UBA agent to $ubaAgent"
